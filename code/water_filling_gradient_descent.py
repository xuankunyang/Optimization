...
def backtracking_line_search(x, direction ,total_waterm, beta = 0.5, max_iter = 1000):
    ...
    for i in range(max_iter):
        if np.sum(x + step * direction) > total_water:
            step = step * beta # Take a smaller one
        else:
            break
...
def fill_water(alpha, total_water, precision, max_iter, use_line_search = True, lr = 1e-3, F_track = False):
    n = len(alpha)
...
def visualize_water(alpha, x, horizontal_line, iteration):
    ...
    plt.title("Visualization of Gradient Descent Method")
    plt.show()
...
def visualize_monkey_search(alpha, monkey_amount, optimal):
    ...
    plt.title("Mokey Search VS Gradient Descent Method")
    plt.show()
...
if __name__ == "__main__":
    ...
    x = fill_water(alpha=alpha, total_water=total_water, precision=precision, max_iter=iteration, use_line_search= True, lr = learning_rate)
    print(x)
    ...
    track_x, track_target = fill_water(alpha=alpha, total_water=total_water, precision=precision, max_iter=iteration, use_line_search= True, lr = learning_rate, F_track=True)
